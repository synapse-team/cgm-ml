...
def get_paths(root_path="../data/preprocessed"):
    dataset_paths = glob.glob(os.path.join(os.path.join(root_path, "*"), "*"))
...
def main():
    ...
    global dataset_path
    global timestamp
    dataset_path = get_dataset_path("../processed-data")
    ...
    preprocessed_path = os.path.join("../preprocessed_trimmed_twice", timestamp)
...
def get_qrcodes_dictionary(qrcodes):
    ...
    for qrcode in qrcodes:
        ...
        for measurement_path in measurement_paths:
            # Getting PCDs.
            pcd_paths = glob.glob(os.path.join(measurement_path, "pcd", "*_neg_neg.pcd"))
            if len(pcd_paths) == 0:
                continue
